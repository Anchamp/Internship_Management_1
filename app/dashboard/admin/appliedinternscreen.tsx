...
// Update the InternshipPost interface to properly type the location field
...
export default function AppliedInternScreen() {
  ...
  // New states for the internship listing view
  const [showInternshipsList, setShowInternshipsList] = useState(true);
  const [selectedInternshipDetails, setSelectedInternshipDetails] =
  ...
  useEffect(() => {
    if (internships.length > 0 && !showInternshipsList) {
      fetchApplications();
    }
  }, [
    internships,
  ...
  const fetchInternships = async () => {
    try {
      ...
      // Instead of making a separate API call for counts that doesn't exist,
      // we'll use the applications data from the internships themselves
      const internshipsWithCounts = internshipsData.internships.map(
  ...
  const fetchApplications = async () => {
    try {
      ...
      // Build query parameters - now always include internshipId if viewing specific internship
      let url = `/api/internships/applications?organizationAdmin=${user.username}`;

      // If we're viewing a specific internship, always filter by that internship
      if (selectedInternshipDetails) {
        url += `&internshipId=${selectedInternshipDetails._id}`;
      } else if (selectedInternship !== "all") {
  ...
  const handleStatusUpdate = async (
    ...
    try {
      ...
      const response = await fetch(
        `/api/internships/applications/${applicationId}/status`,
        {
          method: "PUT",
          headers: {
  ...
  // New function to handle selecting an internship
  const handleSelectInternship = (internship: InternshipPost) => {
    setSelectedInternshipDetails(internship);
    setSelectedInternship(internship._id);
    setShowInternshipsList(false);
    setSearchQuery("");
    setSelectedStatus("all");
  };

  // New function to go back to internships list
  const handleBackToInternships = () => {
    setSelectedInternshipDetails(null);
    setShowInternshipsList(true);
  };
  ...
  // Helper function to format location based on whether it's a string or object
  const formatLocation = (location: any): string => {
    if (!location) return "Remote/Various";
    ...
    if (typeof location === "object") {
      ...
      return parts.length > 0
        ? parts.join(", ")
        : location.address || "Location details available";
    }
    ...
    return (
      <div className="flex flex-col items-center justify-center p-8">
        ...
        <p className="text-gray-500">
          Loading {showInternshipsList ? "internships" : "applications"}...
        </p>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="bg-red-50 p-6 rounded-lg text-center">
        ...
        <button
          onClick={showInternshipsList ? fetchInternships : fetchApplications}
      ...
      <div className="space-y-6">
        ...
        <div className="bg-white rounded-lg shadow-md p-6">
          <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 mb-6">
            <div>
              ...
              <p className="text-gray-600 text-sm mt-1">
                Select an internship to view applications
              </p>
            </div>
            <div className="text-sm text-gray-500">
              {internships.length} internship
              {internships.length !== 1 ? "s" : ""} posted
            </div>
          </div>
          ...
          <div className="relative max-w-md">
            ...
            <input
              type="text"
              className="block w-full pl-10 pr-3 py-2 border border-gray-300 rounded-md focus:ring-cyan-500 focus:border-cyan-500 text-black placeholder-black placeholder-opacity-70"
              placeholder="Search internships..."
              value={internshipsSearchQuery}
              onChange={(e) => setInternshipsSearchQuery(e.target.value)}
            />
          </div>
        </div>
        ...
        {filteredInternships.length === 0 ? (
          <div className="bg-white rounded-lg shadow-md p-12 text-center">
            ...
            <p className="text-gray-500">
              {internships.length === 0
                ...
                : "Try adjusting your search to find more internships."}
            </p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredInternships.map((internship) => (
              <div
                key={internship._id}
                className="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-shadow border border-gray-200 cursor-pointer"
                ...
                <div className="p-4">
                  ...
                  {internship.location && (
                    <div className="flex items-center text-gray-700 mb-2">
                      ...
                      <span>{formatLocation(internship.location)}</span>
                    </div>
                  )}
                  <div className="mt-3 flex items-center justify-between">
                    <div className="flex items-center text-cyan-600">
                      ...
                      <span className="font-semibold">
                        {internship.applicationsCount || 0} applicant
                        {internship.applicationsCount !== 1 ? "s" : ""}
                      </span>
                    </div>
                    <button
                      className="px-3 py-1 bg-cyan-600 text-white rounded-md hover:bg-cyan-700 text-sm flex items-center"
                      onClick={(e) => {
                        e.stopPropagation();
  ...
  // Applications List View (when internship is selected)
  return (
    <div className="space-y-6">
      {/* Header and Filters */}
      <div className="bg-white rounded-lg shadow-md p-6">
            ...
            <div className="bg-gradient-to-r from-cyan-50 to-blue-50 p-4 rounded-lg">
              ...
              <div className="flex flex-wrap gap-x-6 gap-y-2 text-sm text-gray-600">
                <div className="flex items-center">
                  ...
                  {selectedInternshipDetails.department}
                </div>
                {selectedInternshipDetails.location && (
                  <div className="flex items-center">
                    <Mail className="h-4 w-4 mr-2" />
                    {formatLocation(selectedInternshipDetails.location)}
                  </div>
                )}
                <div className="flex items-center">
          ...
          <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
            <div>
              <h3 className="text-lg font-semibold text-gray-900 flex items-center">
                <UserSearch className="mr-2 h-5 w-5 text-cyan-600" />
                Applied Interns
              </h3>
            </div>
          </div>
      ...
      {filteredApplications.length === 0 ? (
        <div className="bg-white rounded-lg shadow-md p-12 text-center">
          ...
          <p className="text-gray-500">
            {applications.length === 0
              ? "Applications will appear here once students start applying to this internship."
                    ...
                    <div className="space-y-3">
                      <div className="flex items-center text-sm">
                        ...
                        <span className="ml-2 text-gray-900">
                          {selectedApplication.userProfileSnapshot.fullName}
                        </span>
                      </div>
                      <div className="flex items-center text-sm">
                        <Mail className="h-4 w-4 mr-2 text-gray-500" />
                        ...
                        <span className="ml-2 text-gray-900">
                          {selectedApplication.userProfileSnapshot.email}
                        </span>
                      </div>
                      <div className="flex items-center text-sm">
                        ...
                        <span className="ml-2 text-gray-900">
                          {selectedApplication.userProfileSnapshot.phone}
                        </span>
                      </div>
                    </div>
                    <div className="space-y-3">
                      <div className="flex items-center text-sm">
                        ...
                        <span className="ml-2 text-gray-900">
                          {selectedApplication.userProfileSnapshot.university}
                        </span>
                      </div>
                      <div className="flex items-center text-sm">
                        <Briefcase className="h-4 w-4 mr-2 text-gray-500" />
                        ...
                        <span className="ml-2 text-gray-900">
                          {selectedApplication.userProfileSnapshot.major}
                        </span>
                      </div>
                      <div className="flex items-center text-sm">
                        ...
                        <span className="ml-2 text-gray-900">
                          {
                            selectedApplication.userProfileSnapshot
                              .graduationYear
                          }
                        </span>
                      </div>
                ...
                {selectedApplication.status === "selected" && (
                  <div className="bg-yellow-50 p-4 rounded-lg border border-yellow-200">
                    ...
                    <p className="text-yellow-700 text-sm">
                      The intern has been selected and is awaiting their
                      response to accept or decline the offer.
                    </p>
                  </div>
                )}
              ...
              <div className="flex justify-between items-center">
                ...
                <div className="flex space-x-3">
                  ...
                  {selectedApplication.status === "interview_scheduled" && (
                    <button
                      onClick={() =>
                        handleStatusUpdate(selectedApplication._id, "selected")
                      }
                      disabled={isUpdatingStatus}
                      className="px-4 py-2 bg-green-600 text-white rounded-md hover:bg-green-700 font-medium disabled:opacity-50"
                    >
                  ...
                  {(selectedApplication.status === "accepted" ||
                    ...
                    <div className="text-sm text-gray-600">
                      No further actions available - intern has responded to the
                      offer.
                    </div>
                  )}
